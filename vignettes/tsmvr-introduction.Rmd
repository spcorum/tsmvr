...
library(tsmvr)
gd_gd_solution = tsmvr::tsmvr_solve(
...
```{r gridsearch, eval = F}
s1_grid = round(p*q*exp(log(seq(0.1,0.2,length.out = 8))))
# s2_grid = round(seq(q,4*q,length.out = 10))
s2_grid = round(seq(2*q,4*q,length.out = 8))
set.seed(5)
grid = tsmvr_gridsearch(
  X = data$X, Y = data$Y, s1_grid = s1_grid, s2_grid = s2_grid, 
  k = 2, reps = 20, Omega_type = 'gd', eta1 = 0.001, eta2 = 0.001,
  quiet = F
)
```
