...
arma::mat cgd(arma::mat &B0, const arma::mat &S, const arma::mat &H, int maxiter=100,
     ...
     *  S, p-by-p matrix
     *  H, p-by-q matrix
     *  maxiter, number of iterations (int)
     *
     * Returns:
...
//' @param BType type of descent for regression steps (string: 'gd')
//' @param OmegaType (string: 'gd' or 'min')
...
//' @param maxiter maximum number of iterations (positive integer)
...
//' 
//' \code{B.hat} - final iterate of the regression matrix \cr
//' \code{Omega.hat} - final iterate of the precision matrix \cr
//' \code{objective} - final value of the objective function \cr
//' \code{B.history} - list of all regression matrix iterates \cr
//' \code{Omega.history} - list of all precision matrix iterates \cr
//' \code{objective.history} - list of the objective function values for each iteration \cr
//' \code{iterations} - number of iterations \cr
//' \code{time} - algorithm time (seconds) \cr
//' \code{Y.hat} - fitted response, given by \code{X*B.hat} \cr
...
List tsmvr_solve(const arma::mat &X, const arma::mat &Y, const int &s1, 
                 const int &s2, 
                 const String &BType = "gd", const String &OmegaType = "gd",
                 const double &eta1 = 0.001, const double &eta2 = 0.001,
                 const double &epsilon = 1e-4, const int &maxiter = 1000,
    ...
    field<arma::mat> BHist(maxiter);
    field<arma::mat> OmegaHist(maxiter);
    field<double> objHist(maxiter);
    ...
    if (!quiet) {
        ...
        //      << ")" << endl;
        // Rcpp::Rcout << BType << "|" << OmegaStep << endl;
    ...
    for (int k=1; k<=maxiter; k=k+1) {
        ...
        // B = ht(updateB(B,Omega,X,Y,S,H,BType,eta1,cgdstep,cgdeps),s1);
        B = ht(updateB(B,Omega,X,Y,S,H,BType,eta1),s1);
        Omega = ht(updateOmega(B,Omega,X,Y,OmegaType,eta2),s2);
    ...
    // Return.
    return List::create(Named("B.hat") = B, 
                        Named("Omega.hat") = Omega,
                        Named("objective") = obj,
                        Named("iterations") = itrs,
                        Named("time") = time,
                        Named("Y.hat") = fv,
                        Named("residuals") = res,
                        Named("B.history") = BHist2,
                        Named("Omega.history") = OmegaHist2,
                        Named("objective.history") = objHist2);
}